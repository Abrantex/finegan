...
def define_optimizers(netG, netsD):
    ...
    for i in range(num_Ds):
        if i == 1:
            ...
            opt = optim.Adam([
                {'params': netsD[i].module.jointConv.parameters()},
                {'params': netsD[i].module.logits.parameters()}],
                lr=cfg.TRAIN.GENERATOR_LR,
                betas=(0.5, 0.999))
            optimizerG.append(opt)
...
class FineGAN_trainer(object):
    ...
    def train_Dnet(self, idx, count):
        '''
         Discriminator is only trained in 
         background and child stage. (NOT in parent stage)
        '''
        if idx == 0 or idx == 2:
            flag = count % 100
            ...
            if idx == 2:
                ...
                for i in range(batch_size):
                    ...
                    a1 = max(torch.tensor(0).float().cuda(), torch.ceil(
                        (x1 - self.recp_field)/self.patch_stride))
                    a2 = min(torch.tensor(self.n_out - 1).float().cuda(),
                             torch.floor((self.n_out - 1) - ((126 - self.recp_field) - x2)/self.patch_stride)) + 1
                    b1 = max(torch.tensor(0).float().cuda(), torch.ceil(
                        (y1 - self.recp_field)/self.patch_stride))
                    b2 = min(torch.tensor(self.n_out - 1).float().cuda(), torch.floor(
                        (self.n_out - 1) - ((126 - self.recp_field) - y2)/self.patch_stride)) + 1

                    if (x1 != x2 and y1 != y2):
                        weights_real[i, :, a1.type(torch.int): a2.type(
                            torch.int), b1.type(torch.int): b2.type(torch.int)] = 0.0

                norm_fact_real = weights_real.sum()
                norm_fact_fake = weights_real.shape[0]*weights_real.shape[1] * \
                    weights_real.shape[2]*weights_real.shape[3]
            ...
            if idx == 0:  # Background stage
                ...
                # Masking output units which correspond to receptive fields
                # which lie within the boundin box
                errD_real_uncond = torch.mul(errD_real_uncond, weights_real)
                errD_real_uncond = errD_real_uncond.mean()

                # Background/foreground classification loss
                errD_real_uncond_classi = criterion(
                    real_logits[0], weights_real)
                ...
                # Normalizing the real/fake loss for background after
                # accounting the number of masked members in the output.
                if (norm_fact_real > 0):
            ...
            if idx == 2:
                ...
                # Real/Fake loss for the real image
                errD_real = criterion_one(real_logits[1], real_labels)
                # Real/Fake loss for the fake image
    ...
    def train(self):
        ...
        hard_noise = \
            Variable(torch.FloatTensor(
                self.batch_size, nz).normal_(0, 1)).cuda()

        # Receptive field stride given the current discriminator
        # architecture for background stage
        self.patch_stride = float(4)
        # Output size of the discriminator at the background stage;
        # N X N where N = 24
        self.n_out = 24
